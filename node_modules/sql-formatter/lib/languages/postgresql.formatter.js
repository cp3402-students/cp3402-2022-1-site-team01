...
_defineProperty(PostgreSqlFormatter, "reservedCommands", reservedCommands);

_defineProperty(PostgreSqlFormatter, "reservedBinaryCommands", reservedBinaryCommands);

_defineProperty(PostgreSqlFormatter, "reservedDependentClauses", reservedDependentClauses);

_defineProperty(PostgreSqlFormatter, "reservedJoinConditions", ['ON', 'USING']);

_defineProperty(PostgreSqlFormatter, "reservedLogicalOperators", ['AND', 'OR']);

_defineProperty(PostgreSqlFormatter, "reservedKeywords", (0, _utils.dedupe)([].concat(_toConsumableArray(Object.values(reservedFunctions).reduce(function (acc, arr) {
  return [].concat(_toConsumableArray(acc), _toConsumableArray(arr));
}, [])), reservedKeywords)));

_defineProperty(PostgreSqlFormatter, "stringTypes", ["\"\"", "''", "U&''", 'U&""', '$$', '``', "E''"]);

_defineProperty(PostgreSqlFormatter, "blockStart", ['(', 'CASE']);

_defineProperty(PostgreSqlFormatter, "blockEnd", [')', 'END']);

_defineProperty(PostgreSqlFormatter, "indexedPlaceholderTypes", ['$']);

_defineProperty(PostgreSqlFormatter, "namedPlaceholderTypes", [':']);

_defineProperty(PostgreSqlFormatter, "lineCommentTypes", ['--']);
